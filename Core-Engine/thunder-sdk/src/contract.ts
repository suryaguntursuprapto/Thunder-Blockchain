...
export class Contract {
    ...
    constructor(
        ...
        public readonly provider: ThunderProvider,
     ...
     */
    static async deploy(
        provider: ThunderProvider,
        wallet: Wallet,
        compiledBytecodeHex: string,
        ...
        const tx = new Transaction(
            ...
            nonce,
            wallet.address,
            new Uint8Array(20), // zero address for deployment
            0n, // value
            dataBytes,
            gasLimit,
            gasPrice,
     ...
     */
    async read(dataHex: string): Promise<string> {
        let pubKey = "";
        if (this.wallet) {
     ...
     */
    async send(dataHex: string, value: bigint = 0n, gasLimit: bigint = 50000n, gasPrice: bigint = 10n): Promise<string> {
        if (!this.wallet) throw new Error("Wallet is required to send state-modifying transactions");
